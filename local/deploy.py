...
def deploy_to_machine(mid, config_only=False):
    ...
    if not config_only:
        deploy_lines.extend([
            '',
            '# Find python3',
            'PY=$(command -v python3 2>/dev/null || ls /nix/store/*/bin/python3 2>/dev/null | grep 3.11 | head -1)',
            '[ -z "$PY" ] && PY=$(ls /nix/store/*/bin/python3 2>/dev/null | head -1)',
            'ln -sf "$PY" /tmp/python3 2>/dev/null || true',
            'PY="${PY:-/tmp/python3}"',
            '',
            '# Start link_server',
            ...
            'while true; do',
            '    sleep 60',
            '    PY=$(command -v python3 2>/dev/null || echo /tmp/python3)',
